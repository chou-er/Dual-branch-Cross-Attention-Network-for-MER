...
from model2 import mobile_vit_x_small
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        # padding
        # 如果输入图片的H，W不是patch_size的整数倍，需要进行padding     ，官网有介绍7分48
...
#patch_size = 4意为将图片4倍下采样，embed_dim=96意为C，depths是依次重复BLOCK的倍数，num_heads是多少个头,window_size是卷积核的大小
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule，产生针对每一个BLOCK的drop_path_rate
...
class SwinTransformer_1(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # stochastic depth
        dpr = [x1.item() for x1 in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule，产生针对每一个BLOCK的drop_path_rate
...
class SwinTransformer_2(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=3,
        ...
        # stochastic depth
        dpr = [x2.item() for x2 in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule，产生针对每一个BLOCK的drop_path_rate
    ...
    def forward(self, x2):
        ...
        x2 = self.norm(x2)  # [B, L, C]                         [8,49,768]
