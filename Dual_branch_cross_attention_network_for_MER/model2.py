...
# import sys
# sys.path.append("E:/My_Work/Second_2flow/Mobile/")
...
class MobileViTBlock(nn.Module):
    """
    This class defines the `Mobile block <https://arxiv.org/abs/2110.02178?context=cs.LG>`_
    ...
    Args:
        ...
        conv_ksize (int): Kernel size to learn local representations in Mobile block. Default: 3
...
class MobileViT(nn.Module):
    """
    This class implements the `Mobile architecture <https://arxiv.org/abs/2110.02178?context=cs.LG>`_
    """
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        # self.reshape.add.module(name="global_pool", module=nn.AdaptiveAvgPool2d(1))
        # self.reshape()
    ...
    def forward(self, x: Tensor) -> Tuple[Any, Any]:
        ...
        x = self.layer_1(x)
        x = self.layer_2(x)
        # x_layer2 = x = self.layer_2(x)
        # x_layer2 = x_layer2.flatten(2).transpose(1, 2)
        # patch2提取的地方
        ...
        x = x.flatten(2)             #[8,384,49]
        x = self.avgpool(x)         #[8,1,384]
        x = x.transpose(1, 2)
        # x = self.reshape(x)
        return x
        # return x_layer2, x
